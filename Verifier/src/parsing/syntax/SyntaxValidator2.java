...
public class SyntaxValidator2 {
	 ...
	 * @throws SyntaxException2       if there is illegal syntax in the file
	 */
	public static Scanner validate(File source) throws FileNotFoundException, SyntaxException2 {
	 ...
	 * @throws SyntaxException2 if there is any illegal syntax in place.
	 */
	private static void searchForMissingSyntax(Scanner sourceFile) throws SyntaxException2 {
		String currentLine;
			...
			for (int i = 0; i < lineAsCharArray.length; i++) {
				switch (lineAsCharArray[i]) {
					...
					case (CURLY_OPEN): // may only be last char
						if (i != lineAsCharArray.length - 1) throw new SyntaxException2();
						else curlyBracketCounter++;
						break;
					case (CURLY_CLOSE): // has to have its own line
						if (lineAsCharArray.length != 1) throw new SyntaxException2();
						else curlyBracketCounter--;
						break;
					case (SEMICOLON_END):
						if (i != lineAsCharArray.length - 1) throw new SyntaxException2();
				...
				if (bracketCounter < 0 || curlyBracketCounter < 0) {
					throw new SyntaxException2(); // no opening brackets
				}
		...
		if (curlyBracketCounter != 0) throw new SyntaxException2();
	}
}